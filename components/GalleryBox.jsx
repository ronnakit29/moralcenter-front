    ...
    return (
        <div>
            <div className="mb-5 grid lg:grid-cols-4 md:grid-cols-4 sm:grid-cols-2 grid-cols-2  gap-4">
                {items.map((i, key) => <img key={key} className='transition-all object-fill max-h-60 hover:scale-105 h-44 bg-cover bg-center rounded-3xl transform cursor-pointer' style={{ backgroundImage: `url(${i})`}} onClick={() => handlerShow(key)} />)}
            </div>
            {show && <div className='fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-30 flex items-center justify-center z-50'  >
                <div id="close" className='w-screen h-screen absolute z-0' onClick={()=>setShow(false)}></div>
                <img src={items[current]} className="max-h-[80vh] z-10 p-3 rounded-3xl bg-white animate__animated animate__fadeIn animate__faster" alt="" />
