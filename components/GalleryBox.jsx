    ...
    return (
        <div>
            <div className="mb-5 grid lg:grid-cols-4 md:grid-cols-4 sm:grid-cols-2 grid-cols-2  gap-4">
                {items.map((i, key) => <div key={key} className='transition-all hover:scale-105 h-44 bg-cover bg-center rounded-3xl transform cursor-pointer' style={{ backgroundImage: `url(${i})`}} onClick={() => handlerShow(key)} />)}
            </div>
