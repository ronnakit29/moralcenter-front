...
export default function CategoryList({ bg, size, activeRoute, categories }) {
    ...
    return (
        <div
            className={`${bg || "bg-gray-100"} rounded-3xl p-5 w-full ${data.length > 5 ? 'lg:flex grid grid-cols-2 md:grid-cols-3' : 'grid grid-cols-5'} justify-between gap-2 lg:gap-10`}
        >
            {/* {JSON.stringify()} */}
            {data.map((i, key) => <div key={key}>
                ...
                {activeRoute == i.id && <div className="rounded-full  w-full h-4 mt-3 " style={{background: i.get('color')}}></div>}
            </div>)}
